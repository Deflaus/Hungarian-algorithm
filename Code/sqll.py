...
# WORKERS ACCESS
    ...
    def insert_workers(id, name, position):
        try:
            ...
            cur.execute('INSERT INTO workers VALUES(' + id + ',"' + name + '","' + position + '") ')
            con.commit()
    ...
    def get_time():
        ...
        for rowsW in workers_table:
            ...
            for rowsT in tasks_table:
                if (countC == len(tasks_table)): continue
        ...
        try:
            ...
            for i, worker in enumerate(workers):
                for j, task in enumerate(tasks):
                    cur.execute('INSERT INTO time VALUES(' + str(worker[0]) + ',' + str(task[0]) + ',' + str(data[i][j]) + ') ')
                    con.commit()
